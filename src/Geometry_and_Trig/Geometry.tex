...
If \( A \), \( B \), and \( C \) are points on a circle where the line \( AC \) is the diameter, 
...
side. Every triangle has three medians, and they intersect at a single point called the \emph{centroid}, which is the triangle’s center of mass. The centroid divides each median in a ratio of \( 2:1 \), with the longer part closer to the vertex.
...
which is the center of the triangle's incircle (the largest circle that fits inside the triangle and touches all three sides).
...
two parallel lines. They are based on similar triangles and allow us to calculate unknown lengths using proportions.
