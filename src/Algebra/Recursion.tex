...
\begin{itemize}[label=\(-\)]
    \item A \textbf{recursive form} defines \( a_n \) based on previous terms (e.g., \( a_n = a_{n-1} + 2 \)).
    \item A \textbf{closed form} expresses \( a_n \) directly in terms of \( n \), with no reference to previous terms.
\end{itemize}
