...
\subsection*{Step 1: Differentiate implicitly}
...
\begin{itemize}
    ...
    \item \(f\) has a local maximum if \(f_{xx}(a_0) < 0 \text{and} \det(H) > 0 \)

    \item \(f\) has a local minimum if \(f_{xx}(a_0) > 0 \text{and} \det(H) > 0 \)

    \item \(f\) has a local saddle  if \(\det(H) < 0\)

    \item The test is inconclusive if \(\det(H) = 0\)
