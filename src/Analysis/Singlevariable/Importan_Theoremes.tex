...
\draw[->] (-0.5,0) -- (5.5,0) node[right] {\emph{x}};
...
\node at (5.3,1.65) {\(N\)};
\node at (1.2,-0.3) {\emph{a}};
\node at (4.2,-0.3) {\emph{b}};
\node at (3,-0.3) {\(c\)};
...
If \emph{f} is continuous on the closed interval \([a, b] \), then \emph{f} attains a maximum and a 
...
Let \emph{f} be continuous on \([a, b] \) and differentiable on \((a, b) \).  
