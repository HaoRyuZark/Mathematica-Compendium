...
\subsubsection*{Example: Convergence Exercise}
...
\begin{itemize}[label=\(-\)]
\item The \textbf{supremum} of a set \(A \subseteq \mathbb{R}\) is the least upper bound: the smallest real number \(s\) such that \(a \le s\) for all \(a \in A\).
\item The \textbf{infimum} of a set \(A\) is the greatest lower bound: the largest number \(t\) such that \(a \ge t\) for all \(a \in A\).
\item The \textbf{maximum} is the largest element in the set, if it exists.
\item The \textbf{minimum} is the smallest element in the set, if it exists.
\end{itemize}
...
\textbf{Theorem:} Every bounded monotonic sequence converges.
