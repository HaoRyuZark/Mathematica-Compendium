...
\textbf{Example:}
\vspace{\baselineskip}
 Let \(f: \mathbb{R} \to \mathbb{R}\) be defined by \(f(x) = x^2\) and \(g: \mathbb{R} \rightarrow \mathbb{R}\) be defined by \(g(x) = x+3\). Then:
\[
...
The inverse of a function \(f: A \to B\) is a function \(f^{-1}: B \rightarrow A\) that 
...
element \(b \in B\), then the inverse function \(f^{-1}\) maps \(b\) back to \(a\).
 ...
 Formally, a function \(f: A \to B\) has an inverse \(f^{-1}: B \rightarrow A\) 
